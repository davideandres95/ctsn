...
# This file was *autogenerated* from the file 18_RS_Decoding.sage
from sage.all_cmdline import *   # import sage library

_sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_1000 = Integer(1000); _sage_const_3 = Integer(3); _sage_const_2 = Integer(2)
import time

def get_column_multipliers(n, alphas, vs_G):
	"""Returns columns multipliers of H for given column multipliers of G"""
	A = matrix(F, n-_sage_const_1 , n, lambda i,j: alphas[j]**i*vs_G[j])
...
# Parameters
q = next_prime(_sage_const_1000 ) 			# Choose a prime power
n = q-_sage_const_1 			# n<q
k = floor(n/_sage_const_3 )		# k<n (here k ~ n/3)
d = n-k+_sage_const_1 		# RS codes are MDS codes
t = floor((n-k)/_sage_const_2 )	# Unique error-correction radius

# Choose alphas
F      = GF(q, 'a')	# Construct a finite field of q elements
alphas = F.list()[_sage_const_1 :]	# Choose alphas to be all non-zero field elements
vs_G   = [F(_sage_const_1 ) for i in range(n)]		  # Column multipliers (of G) v_i'=1
...
print(C)
print('n-k+1 =', n-k+_sage_const_1 )			# Print n-k+1
...
r = c+e
error_pos = [i for i in range(_sage_const_0 , n) if not e[i].is_zero()]
print('u =', u)
print('c =', c)
print('e =', e)
print('r =', r)
...
ELP = prod([_sage_const_1 -alphas[i]*x for i in error_pos])	# Compute ELP (note that a receiver cannot do this since the error positions are unknown)
EEP = sum([e[i]*vs_H[i]*prod([_sage_const_1 -alphas[j]*x for j in error_pos if i!=j]) for i in error_pos])	# Compute EEP (note that a receiver cannot do this since the error positions are unknown)
print('Lambda(x) =', ELP)
print('Omega(x)  =', EEP)
print('S(x)      =', SPoly)
...
print('x^(d-1) =', x**(d-_sage_const_1 ))

# Obtain ELP and error positions from key equation (only works for exactly t errors)
before = time.time()	# start timer
print('\n########################################################\n')
S = matrix(F, t, t, lambda i,j: s[t-_sage_const_1 -j+i])	# Set up syndrome matrix S (as in the lecture notes)
print('S = ')
print(S)
...
est_ELP = Polys([_sage_const_1 ]+lambdavec.list())		# Compute Lambda(x)
print('Lambda(x)      =', ELP)
print('est. Lambda(x) =', est_ELP)		# Compare the actual Lambda(x) and the estimated Lambda(x) given by solving the key equation
est_error_pos = [l for l in range(n) if est_ELP(alphas[l]**-_sage_const_1 )==_sage_const_0 ]	# Find the error positions by evaluating Lambda(x) at all alphas[l]^-1
...
print('\n########################################################\n')
est_EEP = (SPoly*est_ELP).quo_rem(x**(d-_sage_const_1 ))[_sage_const_1 ]	# Omega(x) = (Lambda(x)*S(x) mod x^(d-1)) (Note that a.quo_rem(b) returns the quotient and remainder of the polynomial division a/b, a.quo_rem(b)[1] returns only the remainder)
print('Omega(x)      =', EEP)
print('est. Omega(x) =', est_EEP)
est_ELP_der = est_ELP.derivative()		# Compute the formal derivative of ELP
est_e = vector([F(_sage_const_0 )]*n)			# All-zero vector
for l in est_error_pos:				# Apply Forney's formula
	est_e[l] = -alphas[l]/vs_H[l]*est_EEP(alphas[l]**-_sage_const_1 )/est_ELP_der(alphas[l]**-_sage_const_1 )
print('e      =', e)
